...
volatile int time = 0;	/* press time in ms */
int v[N];
		...
		if((LPC_GPIO1->FIOPIN & (1 << pins[i])) == 0){
			...
			switch(i){
				...
				default /* case 4 */: {
					...
					time += 50;		/* incremento tempo pressione ogni 50 ms */
					
					// Se tempo pressione superiore a 3 s -> LED7 blinks
					if(time == 3050){
						LED_On(7);
						enable_timer(0);
					}
		...
		// Joystick SELECT/DOWN/LEFT/RIGHT/UP rilasciato
			...
			switch(i){
					...
					default /* case 4 */: {
						...
						// Se tempo pressione > 3 s -> Stop timer e reset led
						if(time > 3000){
							disable_timer(0);
							reset_timer(0);
						...
						// Rilascio di UP dopo N valori, con pressione sotto 3 s
						else if(count == N){
							result = overflow(v, count);
